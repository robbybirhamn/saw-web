...
$level = getLevel();
?>
<form action="<?= base_url('transaction/update/'.$id);?>" method="POST" id="form">
<div class="row">
    <?php if($level == "TK"){ ?>
    <div class="col-md-3">
        ...
        <select name="store" id="store" class="form-control" readonly>
            <?php foreach($stores as $store){?>
                <option value="<?= $store['kode_store'] ?>" value="<?= $transaction['store_transaction'] == $store['kode_store'] ? "selected" : "" ?>"><?= $store['kode_store']." - ".$store['name_store'] ?></option>
            <?php } ?>
        </select>
    </div>
    <?php }else{?>
    <div class="col-md-3">
        ...
        <select name="store" id="store" class="form-control select2">
            ...
            <?php foreach($stores as $store){?>
                <option value="<?= $store['kode_store'] ?>" <?= $transaction['store_transaction'] == $store['kode_store'] ? "selected" : "" ?>><?= $store['kode_store']." - ".$store['name_store'] ?></option>
            <?php } ?>
        </select>
    </div>
    <?php } ?>
    ...
    <div class="col-md-3">
        ...
        <input type="date" name="date" id="date" class="form-control" value="<?= $transaction['date_transaction'] ?>">
    </div>
</div>
<hr>
<div class="row">
    <div class="col-md-12">
        <table class="table table-bordered">
            <thead>
                <tr>
                    <th>Produk</th>
                    <td colspan="6"><select id="produk" class="form-control produk productElement" style="width:250px"></select></td>
                </tr>
                <tr>
                    <th width="10%">Size</th>
                    <th width="10%">Qty</th>
        ...
        <table class="table table-bordered" style="font-size:10px !important;">
            ...
            <tbody id="tbody-detail">
                ...
                foreach ($transactiondetail as $trd) {
                    $html = '<tr>
                            <td>
                                <button class="btn btn-sm btn-info btn-editt" type="button" style="font-size:9px" data-id="'.$trd['id_transdetail'].'"><i class="fa fa-edit"></i></button>
                                <button class="btn btn-sm btn-danger btn-rm" type="button" style="font-size:9px"><i class="fa fa-trash"></i></button>
                            </td>
                            <td>
                            <input type="hidden" value="'.$trd['price_transdetail'].'" class="price" name="price_product[]">
                            <input type="hidden" value="'.$trd['code_product'].'" class="product" name="product[]"> <span class="label_produk">'.$trd['code_product']."-".$trd['name_product'].'</span></td>';
                            for ($i=27; $i <= 45 ; $i++) {
...
<script>
    ...
    function selectRefresh(){
        ...
        $('.productElement').select2({
            ...
            ajax: {
                url: '<?= base_url('model/getProductWhere/') ?>'+store,
                dataType: 'json'
    ...
    function resetForm()
    {
        var html = `<tr>
                        ...
                        <td>
                            <input type="text" class="form-control lastStok" id="lastStok" name="lastStok[]" value="0">
                        </td>
                        <td>
                            <input type="text" class="form-control price" id="price" name="price[]" value="0">
                        </td>
                        <td>
                            <input type="text" class="form-control subtotal" id="subtotal" name="subtotal[]" value="0">
                        </td>
                        <td>
                            <button class="btn btn-sm btn-primary btn-plus" type="button"><i class="fa fa-plus"></i></button>
                            <button class="btn btn-sm btn-danger btn-remove" type="button"><i class="fa fa-trash"></i></button>
    ...
    function getTotal() {
        ...
        if(spajak == 1)
        {
            // UPDATE ROBBY 11 APRIL
            $('#pajak').val(total*0.11); //pajak 11 %
        ...
        if(size.val() == null)
        {
            ...
            success:function (response) {
                ...
                if(json.stok == 0 || json.stok == null)
                {
                    ...
                    var subtotalVal = (priceVal-potongan)*qtyVal;
    ...
    $(document).ready(function(){
        ...
        $('#tbody-detail').on('click', '.btn-editt', function(){
            var id = $(this).data('id');
            
            $.ajax({
                url:"<?=base_url()?>model/editstock/",
                type:"POST",
                data : { 
                    id : id,
                 },
                success:function (response) {
                    var json = $.parseJSON(response);
                    // console.log(json);
                    $('#tbody-form').html(json.detail);

                    $('#total').val(json.total);

                    $("#produk").select2("trigger", "select", {
                        data: { id: json.product, text:json.product_name }
                    });

                    $('#total').val(json.total);
                    $('#discount').val(json.discount);
                    $('#potongan').val(json.potongan);
                    $('#pajak').val(json.pajak);
                    getSubtotal(this);
                }
            });
            $(this).closest ('tr').remove();
